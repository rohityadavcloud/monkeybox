...
host_ips="192.168.3.113 192.168.3.234"
host_user=root
...
zone_id=`$cli create zone dns1=$dns_ext internaldns1=$dns_int localstorageenabled=$local_storage securitygroupenabled=$enable_sg name=MyZone networktype=Basic | grep ^id\ = | awk '{print $3}'`
...
netoff_id=$($cli list networkofferings name=DefaultSharedNetworkOffering guestiptype=Shared state=Enabled  displaytext="Offering for Shared networks" | grep ^id\ = | awk '{print $3}')
net_id=`$cli create network zoneid=$zone_id name=guestNetworkForBasicZone displaytext=guestNetworkForBasicZone networkofferingid=$netoff_id | grep ^id\ = | awk '{print $3}'`
echo "Created network $net_id for zone" $zone_id
